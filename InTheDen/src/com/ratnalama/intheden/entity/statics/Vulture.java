/**
 * Vulture.java	--A program responsible to represent a vulture - a static entity.
 * @author        Ratna Lama
 * @version       1.0
 * @since         12/09/2018
 */
...
public class Vulture extends StaticEntity {
	private AudioPlayer hiss;

	// Constructor
	public Vulture(Handler handler, float x, float y) {
	...
	@Override
	public void expired() {
		hiss = new AudioPlayer("/audio/vulture.wav");
		hiss.play();
		handler.getWorld().getItemManager().addItem(Item.egg.createNewItem((int) x, (int) y));
		
	} // expired()

	@Override
	public void render(Graphics magicalBrush) {	
		magicalBrush.drawImage(Assets.vulture[0],
